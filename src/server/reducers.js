...
export function receiver(previousState = {}, action) {
  switch (action.type) {
    case BEACON_DISCOVERED:
      previousState[action.machineId] = mergeReceiversDiscovered(
        previousState[action.machineId],
        action,
      )
      return previousState
    case RECEIVER_PING:
      previousState[action.machineId] = mergeReceiversPing(
        previousState[action.machineId],
        action,
      )
      return previousState
...
export function beacon(previousState = {}, action) {
  switch (action.type) {
    case BEACON_DISCOVERED:
      previousState[action.uuid] = mergeBeacons(
        previousState[action.uuid],
        action,
      )
      return previousState
    default:
  ...
  switch (action.type) {
    case BEACON_DISCOVERED:
      ...
      while (
        ...
        previousState[previousState.length - 1].seenAt <
          action.receivedAt - MAX_MEASUREMENT_TIME
      ) {
        previousState.pop()
      }
      return previousState
